	...
	if (is_master_tp(tp)) {
		...
		*/
		...
		if(external_key_tnb==0||external_key_tnb=="0"){
			...
			opts->mp_join_syns.token = xor_token_key_tnb(mpcb->mptcp_rem_token,external_key_tnb);//#THARINDU
		}
		//#THARINDU
		
		pr_info("mptcp_syn_options() xored to
		ken : %d",opts->mp_join_syns.token);
